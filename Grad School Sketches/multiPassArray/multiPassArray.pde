...
float lowMidtones = 90;
float highMidtones = 180;
...
//take the strokeWeight of the points of each color group, multiply by 2
//then add some small number that is minimum visible line length
void proximityTest() {
  if (useHighlights) {
    highlightsLowLimit = highlightsPointSw * 2 + 2;
  }

  if (useMidtones) {
    midLowLimit = midtonesPointSw * 2 + 2;
  }

  if (useShadows) {
    shadowLowLimit = shadowPointSw * 2 + 2;
  }
}
